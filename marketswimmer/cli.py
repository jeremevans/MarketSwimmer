...
def gui(
    ...
    """
    🖥️ Launch the MarketSwimmer GUI application
    ...
    if safe_mode:
        ...
        try:
            ...
            if 'python.exe' in result.stdout:
                console.print("[red]⚠️ MarketSwimmer GUI is already running![/red]")
...
def analyze(
    ...
    if success is True:
        ...
        console.print("[red]❌ Analysis failed. Check the output above for details.[/red]")
        raise typer.Exit(1)
...
def status():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            console.print("[red]❌ Python check failed[/red]")
    except:
        console.print("[red]❌ Python not accessible[/red]")
    ...
    for module_name, class_name in modules_to_check:
        try:
            ...
            console.print(f"  ❌ {module_name} [red](import error: {e})[/red]")
...
def quick_start():
    ...
    steps = [
        ("1️⃣", "Launch GUI", "marketswimmer gui", "Start with the user-friendly interface"),
        ("2️⃣", "Analyze a Stock", "marketswimmer analyze AAPL", "Analyze Apple's owner earnings"),
        ("3️⃣", "View Results", "Check data/ and charts/ folders", "See your analysis results"),
        ("4️⃣", "Try More", "marketswimmer analyze BRK.B", "Analyze Berkshire Hathaway")
    ]
...
def calculate(
    ...
    try:
        ...
        console.print(f"[yellow]⚠️ Owner earnings calculation not yet fully implemented.[/yellow]")
        console.print(f"[yellow]📝 To calculate owner earnings for {ticker.upper()}:[/yellow]")
        ...
        console.print(f"[green]💡 Module loaded: OwnerEarningsCalculator available[/green]")
        
    except ImportError as e:
        console.print(f"[red]❌ Cannot import OwnerEarningsCalculator: {e}[/red]")
...
def visualize(
    ...
    try:
        ...
        if not visualization_available:
            console.print(f"[red]❌ Visualization dependencies not available[/red]")
            console.print(f"[yellow]💡 Install with: pip install matplotlib PyQt6[/yellow]")
            return
            
        console.print(f"[yellow]⚠️ Visualization workflow not yet fully implemented.[/yellow]")
        console.print(f"[yellow]📝 To create visualizations:[/yellow]")
        console.print("  1. Ensure you have calculated owner earnings data")
        console.print("  2. Data should be in data/ directory as CSV files")
        console.print("  3. Charts will be saved to charts/ directory")
        
        console.print(f"[green]💡 Visualization module available for development[/green]")
