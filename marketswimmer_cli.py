...
def gui(
    ...
    script_name = "market_swimmer_gui_nolog.py" if test_mode else "market_swimmer_gui.py"
    
    console.print(f"[green]🚀 Starting MarketSwimmer GUI ({script_name})...[/green]")
    
    if not run_python_script(script_name):
        console.print("[red]❌ Failed to launch GUI. Check your Python installation.[/red]")
        raise typer.Exit(1)
...
def analyze(
    ...
    if charts_only:
        ...
        success = run_python_script("visualize_owner_earnings.py")
    else:
        console.print(f"[cyan]🔄 Running complete analysis for {ticker}...[/cyan]")
        success = run_python_script("analyze_ticker.py", [ticker])
