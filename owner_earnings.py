...
class OwnerEarningsCalculator:
    ...
    def __init__(self, xlsx_file_path):
        """
        Initialize with path to XLSX file containing financial statements.
    ...
    def load_financial_statements(self):
        ...
        try:
            print(f"📊 Loading financial data from: {os.path.basename(self.file_path)}")
            ...
            if income_sheet:
                ...
                print(f"✅ Loaded Income Statement: {income_sheet}")
                print(f"   📊 Shape: {self.income_statement.shape}")
            ...
            if balance_sheet:
                ...
                print(f"✅ Loaded Balance Sheet: {balance_sheet}")
                print(f"   📊 Shape: {self.balance_sheet.shape}")
            ...
            if cashflow_sheet:
                ...
                print(f"✅ Loaded Cash Flow Statement: {cashflow_sheet}")
                print(f"   📊 Shape: {self.cash_flow.shape}")
    ...
    def load_financial_statements_by_type(self, data_type):
        ...
        try:
            print(f"📊 Loading {data_type.lower()} financial data from: {os.path.basename(self.file_path)}")
            ...
            print(f"❌ Error loading {data_type.lower()} financial statements: {e}")
            return False
    ...
    def debug_financial_data(self):
        ...
        print(f"\n🔍 DEBUG: Available financial line items...")
        
        if self.income_statement is not None:
            print(f"\n📊 INCOME STATEMENT ({self.income_statement.shape}):")
        ...
        if self.cash_flow is not None:
            print(f"\n💰 CASH FLOW STATEMENT ({self.cash_flow.shape}):")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if not xlsx_file:
            print("❌ No XLSX files found in ./downloaded_files directory")
            print("💡 Usage: python owner_earnings.py <path_to_xlsx_file>")
            print("💡 Or place XLSX files in ./downloaded_files directory")
            return
        
        print(f"📁 Using most recent file: {os.path.basename(xlsx_file)}")
    ...
    for data_type in data_types:
        ...
        if calculator.load_financial_statements_by_type(data_type):
            ...
            if owner_earnings:
                ...
                output_file = f"owner_earnings_{calculator.company_name}_{suffix}.csv"
            ...
            print(f"❌ Failed to load {data_type.lower()} financial statements")
