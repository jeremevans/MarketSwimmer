...
def load_data():
    ...
    try:
        # Load annual data
        annual_df = pd.read_csv('owner_earnings_financials_annual.csv')
        print(f"[OK] Loaded annual data: {len(annual_df)} years")
        
        # Load quarterly data
        quarterly_df = pd.read_csv('owner_earnings_financials_quarterly.csv')
...
def save_and_show_plots(figures, filenames, ticker):
    ...
    for fig, filename in zip(figures, filenames):
        # Replace cvna with actual ticker in lowercase
        actual_filename = filename.replace('cvna', ticker.lower().replace('.', ''))
        filepath = f"{actual_filename}.png"
...
def main():
    ...
    figures.append(fig1)
    filenames.append("cvna_owner_earnings_comparison")
    
    # 2. Components breakdown
    fig2 = create_components_breakdown(annual_df, quarterly_df, ticker)
    figures.append(fig2)
    filenames.append("cvna_earnings_components_breakdown")
    
    # 3. Volatility analysis
    fig3 = create_volatility_analysis(quarterly_df, ticker)
    figures.append(fig3)
    filenames.append("cvna_volatility_analysis")
